            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="relative rounded-tl-md  rounded-tr-md overflow-auto">
                    <div class="overflow-x-auto relative p-2">
                        ...
                        <form method="POST" enctype="multipart/form-data" action="{{ route('admin.my.work.create.action') }}" id="file-upload-form">
                            @csrf
                            <div class="mb-4 items-center">
                                <label for="order_title" class="lable_form">{{ __('work_title') }} <span class="text-red-500">*</span> </label>
                                <input type="text" name="title" id="work_title" class="form_input !w-full" maxlength="255" value="{{ old('title') }}" />
                            </div>
...
<script type="module">
    ...
    async function uploadFormData(formData) {
        try {
            ...
            const response = await fetch(`{{ route('admin.my.work.create.action') }}`, {
                method: 'POST',
                body: formData,
            });
