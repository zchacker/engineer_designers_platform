            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="relative rounded-tl-md  rounded-tr-md overflow-auto p-8">
                    ...
                    <div class="overflow-x-auto relative">
                        ...
                        <form action="{{ route('editor.page.edit.action' , $page->id) }}" method="post" enctype="multipart/form-data" onsubmit="return form_submit(this);" class="w-full">
                            @csrf
                            <div class="mb-4">
                                <label for="title" class="lable_form">{{ __('title') }}</label>
                                <input type="text" name="title" class="form_input !w-full" value="{{  $page->title ?? old('title') }}" required/>
                            </div>
                            
                            <div class="mb-4">
                                <label for="description" class="lable_form">{{ __('description') }}</label>
                                <textarea name="description" id="description" cols="30" rows="4" class="form_input !w-full">{{$page->description ?? old('description')}}</textarea>                                
                            </div>                            

                            <div class="mb-4">
                                <label for="path" class="lable_form">{{ __('path') }}</label>
                                <input type="text" name="path" class="form_input !w-full" value="{{ $page->path ?? old('path') }}" required/>
                            </div>
